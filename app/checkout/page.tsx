...
/** 주문서 페이지 — 배송지 입력 + 주문 요약 + 주문 생성 (결제창 호출은 Step 4에서 교체) */
export default function CheckoutPage() {
  ...
  /** 주문 성공 시 채워짐 — 값이 있으면 임시 완료 화면으로 전환 */
  ...
  /** 주문 생성 요청 — 검증은 서버(validateShippingInfo)가 담당, 400 메시지를 그대로 표시 */
  const submitOrder = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setOrderResult((await response.json()) as CreateOrderResult);
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        <section className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
          {orderResult ? (
            /* ── 임시 완료 화면 (Step 4에서 토스 결제창 호출로 교체) ── */
            <div className="mx-auto max-w-md py-12 text-center">
              <h1 className="text-2xl font-bold text-gray-900">
                주문이 접수되었습니다
              </h1>
              <p className="mt-3 text-sm text-gray-500">
                결제 기능은 현재 연동 준비 중입니다.
              </p>
              <dl className="mt-8 space-y-3 rounded-xl bg-gray-50 p-5 text-left text-sm">
                <div className="flex justify-between gap-4">
                  ...
                  <dd className="font-medium text-gray-900">{orderResult.orderName}</dd>
                </div>
                <div className="flex justify-between gap-4">
                ...
                <div className="flex justify-between gap-4">
                  <dt className="shrink-0 text-gray-500">결제 예정 금액</dt>
