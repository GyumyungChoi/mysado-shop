...
- 🟢 **승인 경로는 멱등하다** — `confirm/route.ts:57` 이 `status === "PAID"` 를 조기 반환하고
  `:67` 이 `PENDING` 아닌 주문에 409를 낸다. 같은 주문의 confirm 재요청이 **이중 차감·이중
  기록을 만들지 않음**을 96차 라이브에서 실측했다(16:42:48 재요청 200, 변화 0).
- 🔴 **토스 웹훅은 미등록이다**(96차 상점관리자 실측, 라이브 목록 0건). 경로
  `app/api/payment/webhook/route.ts` 는 구현돼 있으나 **한 번도 운영 트래픽을 받은 적이 없다.**
  - 이 라우트는 페이로드를 믿지 않고 `getTossPayment` 로 **역검증**한 뒤 그 결과로만 쓴다(`:50`).
    `CANCELED` 분기는 `:120` 에서 `order.status !== "CANCELED"` 를 보고, 복원은
    `wasPaid` 일 때만(`:123`·`:131`) — **순차 이중 복원은 막혀 있다.**
  - 🟡 **그러나 상태 검사(`:68`·`:92`·`:120`)가 트랜잭션(`:94`·`:124`) 밖**이고
    `tx.order.update` 의 `where` 가 `{ id }` 뿐이라, **웹훅과 우리 API가 겹쳐 도착하면
    이중 차감·이중 복원의 창이 열린다.** confirm·cancel 라우트도 같은 형태지만 **토스 호출이
    순서를 정해 주는 반면, 웹훅은 조회만 하므로 그 심판이 없다.**
  - 📌 **등록 전에 상태 검사를 트랜잭션 안으로 옮긴다**(`updateMany({where:{id, status:<읽은 값>}})`
    후 건수 1 확인). 결제 경로 코드이므로 **단독 배포**한다 → 이월 #104.
  - ⚠️ 웹훅의 역할은 주석 `:10` 대로 **"confirm 성공 후 DB 반영 실패"의 보정망**이다.
    *"없으면 창을 닫은 주문이 PENDING 에 남는다"* 는 **거짓** — confirm 을 우리가 불러야 토스가
    `DONE` 이 되므로 웹훅 유무와 무관하게 `PENDING` 이다(96차 작성자 오류 서른두 번째).
...
- **미결제 주문은 장바구니를 비우지 않는다**(79차 실측). `MYSADO-260902-0001`(PENDING) 생성
  ...
  **결제 승인 경로(`confirm/route.ts:122` 트랜잭션)** 로 확정했다.
