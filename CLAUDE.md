...
- 🎉 **98차에 실시간 계좌이체도 결제·취소를 관통했다**(실측) — `MYSADO-260924-0001` · 토스뱅크 ·
  4,500원 · 재고 5→4→5 · 은행 출금 12:28 → **환불 12:35 실시간**. 취소 요청 본문은 `cancelReason`
  하나뿐(`lib/payment.ts:208`)이고, **`refundReceiveAccount` 없이** `cancelStatus: DONE` 이었다.
  - 🔴 **`refundReceiveAccount` 는 가상계좌를 입금 후 취소할 때만 넣는다**(토스 문서). 카드·계좌이체·
    간편결제 취소 본문에는 넣지 않는다. 한동안 규약에 *"안 보내서 계좌이체 취소가 실패한다"* 로 적혀
    있었으나 **가설이 단정으로 옮겨진 것**이었다(99차 정정).
  - 🟢 **현금영수증은 토스가 발급·취소를 함께 처리한다**(실측) — 우리 코드에 현금영수증 호출은 없고,
    결제를 취소하면 `payload.cashReceipts[]` 에 `transactionType: CANCEL` 이 자동으로 붙는다.
    `issueStatus: IN_PROGRESS` 는 정상이다(토스 문서: 발급 승인은 1~2일 뒤 조회).
    결제창에서 "미발행"을 골라도 `cashReceipt.type = 소득공제` 로 발급됐다 — 상점 자진발급 설정과
    부합한다(**추정**, 상점 설정 미확인). 현금영수증 발급·취소 코드를 새로 쓰기 전에 이 동작부터 확인한다.
  - 🟡 **취소 멱등키는 고정값 `cancel-${paymentKey}` 다**(`lib/payment.ts:206`). 계좌이체 취소에는
    은행 시간·일시 오류 실패(`NOT_AVAILABLE_BANK` 403 · `FAILED_REFUND_PROCESS` 500)가 있고(토스 문서),
    같은 키의 재요청이 **첫 실패를 재생할 수 있다**(**추정** — 문서에 명시 없음). 토스 문서는 에러 뒤
    **키를 바꿔 재시도하는 것도 위험하다**고 하므로, 지시서 없이 이 키의 형태를 바꾸지 않는다.
...
- 🟢 **토스 웹훅은 97차에 등록·개통됐다**(2026-09-23 **16:29** KST · 라이브 `mysadokg25` ·
  ...
  - 🟡 웹훅이 confirm 보다 먼저 PAID로 전이하면 confirm 은 0건 분기로 빠져 **장바구니 정리·확인
    메일이 생략**된다(기존 웹훅 보정 경로와 같은 성질. 97·98차 관측 4건은 모두 0.19~0.26초 뒤에 와서 선착 0건).
...
- 🔴 **가상계좌를 켜는 날의 선행조건**(97차) — confirm 은 승인이 `ok` 면 결제 상태와 무관하게 PAID로
  ...
  메일**이 나간다. 켜기 전에 confirm 의 상태 분기와 `DEPOSIT_CALLBACK` 구독을 먼저 만들고, 입금 후
  취소에 필요한 **`refundReceiveAccount`** 를 cancel 본문에 싣는 경로도 함께 만든다(토스 문서).
...
- 🔴 **보고·주석·문서에 사실을 적을 때 근거의 층을 붙인다**(99차 신설) — **실측**(명령 출력·파일) /
  **문서**(외부 공식 문서) / **추정**. 지시서나 기존 문서에서 옮겨 적을 때는 원문의 확신도를 그대로
  옮기고, *"(가설)"·"가능"* 을 떼어 단정형으로 바꾸지 않는다(96차: 91차 가설 *"환불 계좌를 요구할 수
  있다"* 가 규약에 *"보내지 않아 실패한다"* 로 들어갔고, 98차에 토스 문서와 실결제로 기각됐다).
